...
# ---------------- Child process for Windows (mp) ----------------
def child_login_mp(conn, user_id, username, password):
    class ConnWriter:
        ...
        def write(self, s):
            ...
            while '\n' in self.buf:
                ...
                self.conn.send(("output", line + '\n'))
            if self.buf and '\n' not in self.buf:
                self.conn.send(("output", self.buf))
                self.buf = ''
    ...
    try:
        ...
        otp = input(f" Enter code (6 digits) for {username} (2FA): ")
        try:
        ...
        otp = input(f" Enter code (6 digits) for {username} (Challenge): ")
        try:
            ...
            print(f"[{username}] ❌ Challenge failed: {e}")
    except Exception as e:
...
# ---------------- Reader thread for Unix PTY ----------------
def reader_thread_pty(user_id: int, chat_id: int, master_fd: int, username: str, password: str):
    global APP, LOOP
    buf = b""
    while True:
        try:
            ...
            if conn.poll(0.1):
                ...
                if msg_type == "exit":
                    ...
                    try:
                        if APP and LOOP:
                            asyncio.run_coroutine_threadsafe(
                                APP.bot.send_message(chat_id=chat_id, text=f"🔥{prompt}"), LOOP
                            )
                    except Exception:
                        logging.error("[THREAD] send_message failed")
                    pending_prompt = True
                    with SESSIONS_LOCK:
                        if user_id in SESSIONS:
                            SESSIONS[user_id]['pending_prompt'] = True
        except Exception as e:
            logging.error(f"[THREAD] Unexpected error: {e}")
            break
    try:
        playwright_file = f"sessions/{user_id}_{username}_state.json"
        if os.path.exists(playwright_file):
        ...
        with SESSIONS_LOCK:
            if user_id in SESSIONS:
                if os.name == 'nt':
                    try:
                        SESSIONS[user_id]['conn'].close()
                    except Exception:
                        pass
                    try:
                        SESSIONS[user_id]['process'].join(timeout=5)
                    except Exception:
                        pass
                else:
                    try:
...
async def relay_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.name == 'nt':  # Windows
        if info.get('pending_prompt', False):
            conn = info['conn']
            try:
                conn.send(("input", text.strip()))
                info['pending_prompt'] = False
                await update.message.reply_text("✅ Code sent!")
            except Exception as e:
                await update.message.reply_text(f"Failed to send input: {e}")
        else:
            await update.message.reply_text("⚠️ Session not waiting for input right now.")
    else:  # Unix PTY
...
async def get_password(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if os.name == 'nt':  # Windows
        ...
        process = mp.Process(target=child_login_mp, args=(child_conn, user_id, username, password))
        process.start()
        t = threading.Thread(target=reader_thread_mp, args=(user_id, chat_id, parent_conn, username, password, process), daemon=True)
        t.start()
        with SESSIONS_LOCK:
            SESSIONS[user_id] = {"pid": process.pid, "conn": parent_conn, "process": process, "username": username, "password": password, "chat_id": chat_id, "pending_prompt": False}
    else:  # Unix
        ...
        if pid == 0:
            ...
            t = threading.Thread(target=reader_thread_pty, args=(user_id, chat_id, master_fd, username, password), daemon=True)
            t.start()
            with SESSIONS_LOCK:
